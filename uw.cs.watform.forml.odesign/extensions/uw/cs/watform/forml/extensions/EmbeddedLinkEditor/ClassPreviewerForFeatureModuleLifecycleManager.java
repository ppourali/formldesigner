...
public class ClassPreviewerForFeatureModuleLifecycleManager extends AbstractEEFWidgetLifecycleManager {
	...
	private FeatureModule modelElement;

	public ClassPreviewerForFeatureModuleLifecycleManager(EEFCustomWidgetDescription description, IVariableManager variableManager,
		...
		if (elem instanceof FeatureModule) {
			this.modelElement = ((FeatureModule) elem);
		}
	...
	private void createEditor_V0(Composite parent) {
		...
		try {
			...
			List<Roleable> roleables = new ArrayList<Roleable>();

			if (this.modelElement instanceof FeatureModule && this.modelElement != null) {
				Roleable relatedRoleable = ModelUtils2.getRoleableForFaetureModule((FeatureModule) this.modelElement);
				roleables.add(relatedRoleable);
