...
public class CustomSearchBarForFeatureNode extends Composite {
	...
	public CustomSearchBarForFeatureNode(Composite parent, int style) {
		super(parent, style);
		ConstructDesign();
...
//				}
	...
	protected void ShowCreateDialog() {
		...
		if (sucessfulCreation) {
			...
			FPC_SingletonData.getFeaturesListDialog().doOnSelect();
		}
	}
	...
	private void createMap() {
		...
		List<Feature> features = ModelUtils
				.getWorldModelObjectsByType(FPC_SingletonData.getCurrentRoleable(), Feature.class);
		...
		List<Entity> entities = ModelUtils.getWorldModelObjectsByType(FPC_SingletonData.getCurrentRoleable(),
				Entity.class);
	...
	private void ViewAndHighlightInUI(String name) {
		...
		if (elements == null || elements.isEmpty()) {
			...
			boolean result = FPC_SingletonData.getFeaturesListDialog().findItem(ele);
			if (!result)
