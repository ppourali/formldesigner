...
public class FrmShellForTypedGuardCrossReferenceView_TreeStyle extends AbstractShellContainer {
	...
	final FrmShellForTypedGuardCrossReferenceView_TreeStyle shellContainer = this;
	...
	private CustomCompositeSearchForEveryThing customCompositeSearchForEveryThing;

	private Button btnNewButton;

	public FrmShellForTypedGuardCrossReferenceView_TreeStyle(Display display, List<EObject> crossRefs, EObject elem,
	...
	private void constructDesign() {
		...
		GridData gd_composite_TreeAndEditorHolder = new GridData(SWT.FILL, SWT.FILL, true, false, 2, 2);
		gd_composite_TreeAndEditorHolder.widthHint = 876;
		...
		btnImport.setImage(SWTResourceManager.getImage(FrmShellForTypedGuardCrossReferenceView_TreeStyle.class,
		...
		btnCancel.setImage(SWTResourceManager.getImage(FrmShellForTypedGuardCrossReferenceView_TreeStyle.class,
		...
		btnConfirmDelete.setImage(SWTResourceManager.getImage(FrmShellForTypedGuardCrossReferenceView_TreeStyle.class,
		...
		composite_2.setLayoutData(compositeData);
		formToolkit.adapt(composite_2);
		formToolkit.paintBordersFor(composite_2);

		customCompositeSearchForEveryThing = new CustomCompositeSearchForEveryThing(composite_2, SWT.NONE,
				this.changedAttType);
	...
	protected void openSearchForEveryThingDialogue() {
		...
		final GridData data = (GridData) composite_2.getLayoutData();

		data.exclude = !data.exclude;
		composite_2.setVisible(!data.exclude);
		if (data.exclude == true) {
			// composite_SearchHolder.getParent().pack();
			if (this.getShell() != null)
				this.getShell().setSize(900, 460);

		} else {
			if (this.getShell() != null)
				this.getShell().setSize(900, 565);
		}
