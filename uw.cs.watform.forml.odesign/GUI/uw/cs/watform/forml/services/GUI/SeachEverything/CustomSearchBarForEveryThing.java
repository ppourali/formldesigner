...
public class CustomSearchBarForEveryThing extends Composite {
	...
	CustomCompositeSearchForEveryThing customSearchBarHolder;
	/**
	...
	public CustomSearchBarForEveryThing(Composite parent, int style, EObject anyElement,
			CustomCompositeSearchForEveryThing searchBarHolder) {
		super(parent, style);
		this.customSearchBarHolder = searchBarHolder;
		this.any = anyElement;
		ConstructDesign();
	...
	private void createMap() {
		...
		List<Feature> features = ModelUtils.getWorldModelObjectsByType(this.any, Feature.class);
		...
		List<Entity> entities = ModelUtils.getWorldModelObjectsByType(this.any, Entity.class);
	...
	private void addToTransitionsMap() {
		retrieveTransitionList();

		for (Transition tran : transitionListAsString.keySet()) {
			String name = transitionListAsString.get(tran);
			if (name != "") {
	...
	private void retrieveTransitionList() {
		List<Transition> listOfAllTransitions = ModelUtils.getBehaviourModelObjectsByType(this.any, Transition.class);
		...
		for (Transition t : listOfAllTransitions) {
			...
			transitionListAsString.put(t, itemText + " :: Transition");
	...
	private void ConstructDesign() {
		...
		lblResult.setText("Hit Enter to Find the Element.");
		GridData gd_lblResult = new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1);
		gd_lblResult.widthHint = 170;
		gd_lblResult.heightHint = 15;
	...
	private void ViewAndHighlightInUI(String name) {
		...
		if (elements == null || elements.isEmpty()) {
			...
			boolean result = this.customSearchBarHolder.CreateEditor(ele);
			if (!result)
				lblResult.setText("The element could not be found.");
			else
