...
public class FrmShellForWCAActionView extends AbstractShellContainer {
	...
	private CustomSearchTextForWCA comp_SearchBar;
	...
	public FrmShellForWCAActionView(Display display, Feature containerFeature, List<List<ExprRef>> suggItems,
			EditPart ep) {
		...
		comp_SearchBar = new CustomSearchTextForWCA(shell, this, parentFeature);
	...
	public void populateOperationsList(String strFilter) {
		...
		if (lstOperations.getItemCount() > 0)
			...
			lstOperations.add("Create Output Message in " + this.parentFeature.getName());
			lstOperations.setData("Create Output Message in " + this.parentFeature.getName(), this.parentFeature);

			// lstOperations.add("Create Output Message");
			// lstOperations.setData("Create Output Message", null);

			// lstOperations.add("Create Attribute in " +
			// this.parentFeature.getName());
			// lstOperations.setData("Create Attribute in " +
			// this.parentFeature.getName(), this.parentFeature);
		}
			...
			if (referenceFeature != null) {
				...
				lstOperations.add("Create Output Message in " + parentFeatureName);
				lstOperations.setData("Create Output Message in " + parentFeatureName, referenceFeature);

				// lstOperations.add("Create Attribute in " +
				// parentFeatureName);
				// lstOperations.setData("Create Attribute in " +
				// parentFeatureName, referenceFeature);
			}
		}
	}
	...
	private void constructDesign(Shell shell) {
		...
		shell.setText("Select WCA Action");
	...
	public Object analyzeReturnItem() {
		...
		if (lstOperations.getSelectionCount() > 0) {
			...
			if (selectedText.contains("(")) {
				...
				DataKey = selectedText.substring(0, selectedText.indexOf(":")).toLowerCase().trim();
				retItem = lstOperations.getData(DataKey);
			} else if (selectedText.startsWith("Create Output Message in ")) {
				...
				if (referenceFeature != null) {
					ModelOperations2.addNewOutputMessageToFeature(referenceFeature, newMessage,
			...
			// DataKey = selectedText;
			// Feature referenceFeature = (Feature)
			// lstOperations.getData(DataKey);
			//
			// Attribute newatt = FormlFactory.eINSTANCE.createAttribute();
			// if (referenceFeature != null) {
			// ModelOperations2.addNewAttributeToFeature(referenceFeature,
			// newatt,
			// parseStrFilter(comp_SearchBar.txtFilter.getText().trim())[1]);
			//
			// retItem = newatt;
			// }
			// }
