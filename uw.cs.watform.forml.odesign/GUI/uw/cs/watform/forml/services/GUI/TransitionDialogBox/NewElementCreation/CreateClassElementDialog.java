...
public class CreateClassElementDialog extends Dialog {
	...
	public CreateClassElementDialog(Shell parent) {
		super(parent);
	}
	...
	private void populateComboClass() {
		...
		java.util.List<Roleable> roleables = ModelUtils
				.getWorldModelObjectsByType(CommonSpaceForContextualUI.getCurrentTransition(), Roleable.class);
		...
		for (Roleable r : roleables) {
			...
			if (r.equals(CommonSpaceForContextualUI.getCurrentFeatureOrEntityClass())) {
		...
		try {
			...
			if (roleable instanceof Feature || roleable instanceof Entity) {
				...
				if (comboType.getText().equals("Attribute")) {
					ModelOperations2.CreateNewAttribute(CommonSpaceForContextualUI.getCurrentFeatureOrEntityClass(),
							newElementText);
				} else {
					if (roleable instanceof Feature) {
