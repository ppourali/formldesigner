...
public class CustomSearchBar extends Composite {
	...
	public CustomSearchBar(Composite parent, int style) {
		super(parent, style);
		ConstructDesign();
...
//		}
		...
		// if (resultsArray.length == 0) {
		// resultsArray = new String[] { "Create Element: " + text };
		// btnCreate.setEnabled(true);
		// }
...
//				}
	...
	protected void ShowCreateDialog() {
		CreateElementDialog createDialog = new CreateElementDialog(this.getShell());
		boolean sucessfulCreation = createDialog
				.open(textSearch.getText().replace("Create Element: ", "").trim().replace(" ", ""));
		if (sucessfulCreation) {
			EmbeddedEditorGenerator.UpdateXtextModel(true, true, true, "");
			// EmbeddedEditorGenerator.UpdateXtextModel(Guard.class, "");
			// EmbeddedEditorGenerator.UpdateXtextModel(WCAList.class, null);

			CommonSpaceForContextualUI.getFeaturesListDialog().doOnSelect();
		}
	}

	private void createMap() {
		map_Names_Element.clear();
		List<Feature> features = ModelUtils
				.getWorldModelObjectsByType(CommonSpaceForContextualUI.getCurrentTransition(), Feature.class);
		...
		List<Entity> entities = ModelUtils.getWorldModelObjectsByType(CommonSpaceForContextualUI.getCurrentTransition(),
				Entity.class);
	...
	private void ViewAndHighlightInUI(String name) {
		...
		if (elements == null || elements.isEmpty()) {
			...
			boolean result = CommonSpaceForContextualUI.getFeaturesListDialog().findItem(ele);
			if (!result)
				lblResult.setText("There element could not be found.");
			else
