...
public class XtextEmbeddedEditor {
	...
	BadLocationException {
		...
		if (!parseResult.hasSyntaxErrors() && !hasDanglingRefs(xtextResource)) {
			...
			//xtextResource.getResourceSet().getLoadOptions().putAll(SaveOptions.newBuilder().format().getOptions().toOptionsMap());
			//xtextResource.load(SaveOptions.newBuilder().format().getOptions().toOptionsMap());
...
//		}
	...
	private void setEditorBounds() {
		...
		//xtextEditorComposite.setBounds(bounds.x + 100, bounds.y - 20, width, height);
		//bounds=figure.getBounds().getCopy();
		xtextEditorComposite.setBounds(bounds.x , bounds.y , 300 , 60 );
		
		//xtextEditorComposite.setBounds(bounds.x+2 , bounds.y+27 , width, height);
