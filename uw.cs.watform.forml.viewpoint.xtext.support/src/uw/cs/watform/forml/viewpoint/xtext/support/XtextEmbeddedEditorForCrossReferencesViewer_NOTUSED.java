...
public class XtextEmbeddedEditorForCrossReferencesViewer_NOTUSED {
	...
	private EObject inconsistentObject;

	public XtextEmbeddedEditorForCrossReferencesViewer_NOTUSED(Injector xtextInjector, boolean isEditable,EObject faultyObject) {
		this.xtextInjector = xtextInjector;
		this.isEnabled = isEditable;
		
		this.inconsistentObject=faultyObject;
	}
	 ...
	 * @param isReconcile
	 */
	public boolean closeEditor(boolean isReconcile) {
	...
	protected void createXtextEditor() throws Exception {
		...
		addKeyListener();
		addFocusListener();
		changeEditorColor(isEnabled);
		ChangeColorForSpecificText();
	...
	private void ChangeColorForSpecificText() {

		try {
			//ICompositeNode node = NodeModelUtils.getNode(((Message)this.inconsistentObject).getName());
			String text=((Message)this.inconsistentObject).getName();//NodeModelUtils.getTokenText(node);
			
			Color red = SWTResourceManager.getColor(245, 0, 0);
			int start = this.xTextEmbeddedEditor.getViewer().getTextWidget().getText().indexOf(text);
			int end = start+text.length();

			
			
			StyleRange range = new StyleRange(30, 31, null, red);

			this.xTextEmbeddedEditor.getViewer().getTextWidget().setStyleRange(range);
		} catch (Exception e) {
			System.err.println("could not set the color of the styledText for the embedded editor.");
			e.printStackTrace();
		}
	}
