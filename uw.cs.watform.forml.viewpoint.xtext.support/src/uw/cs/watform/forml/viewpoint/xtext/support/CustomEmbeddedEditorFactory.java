...
public class CustomEmbeddedEditorFactory {
	...
	public static class Builder {
		...
		@Inject
		protected CustomEmbeddedEditorActions.Factory actionFactory;
		...
		public EmbeddedEditor withParent(final Composite parent) {
			...
			final XtextSourceViewer viewer = this.sourceViewerFactory.createSourceViewer(parent, annotationRuler, null, // overviewRuler
					...
					SWT.NONE | SWT.WRAP | SWT.VERTICAL);
			...
			final SourceViewerDecorationSupport viewerDecorationSupport = new SourceViewerDecorationSupport(viewer,
					null, // overviewRuler
			...
			Iterator<AnnotationPreference> e = Iterators
					.filter(annotationPreferences.getAnnotationPreferences().iterator(), AnnotationPreference.class);
			while (e.hasNext()) {
			...
			final CustomEmbeddedEditorActions actions = initializeActions(viewer);
			...
			final EmbeddedEditor result = new EmbeddedEditor(document, viewer, viewerConfiguration, resourceProvider,
					new Runnable() {
						@Override
						public void run() {
				...
				public void processIssues(List<Issue> issues, IProgressMonitor monitor) {
					...
					if (annotationModel != null) {
						if (this.annotationIssueProcessor == null) {
							this.annotationIssueProcessor = new AnnotationIssueProcessor(document, annotationModel,
									new IssueResolutionProvider() {
										...
										public List<IssueResolution> getResolutions(Issue issue) {
											List<IssueResolution> resolutions = issueResolutionProvider
													.getResolutions(issue);
											List<IssueResolution> result = Lists.transform(resolutions,
													new Function<IssueResolution, IssueResolution>() {

														@Override
														public IssueResolution apply(final IssueResolution input) {
															IssueResolution result = new IssueResolution(
																	input.getLabel(), input.getDescription(),
																	input.getImage(), new IModificationContext() {
																		@Override
																		public IXtextDocument getXtextDocument(
																				URI uri) {
																			if (uri.trimFragment()
																					.equals(document.getResourceURI()))
																				return document;
																			return input.getModificationContext()
																					.getXtextDocument(uri);
																		}

																		@Override
																		public IXtextDocument getXtextDocument() {
																			IModificationContext original = input
																					.getModificationContext();
																			if (original instanceof IssueModificationContext) {
																				URI uri = ((IssueModificationContext) original)
																						.getIssue().getUriToProblem();
		...
		protected void afterCreatePartialEditor(XtextSourceViewer viewer, XtextDocument document,
				CompositeRuler verticalRuler, final CustomEmbeddedEditorActions actions) {
			if (verticalRuler != null && annotationTypes != null && annotationTypes.length > 0) {
				AnnotationRulerColumn annotationRulerColumn = new AnnotationRulerColumn(viewer.getAnnotationModel(),
						VERTICAL_RULER_WIDTH, new DefaultMarkerAnnotationAccess() {
							@Override
							public int getLayer(Annotation annotation) {
								if (annotation.isMarkedDeleted()) {
									return IAnnotationAccessExtension.DEFAULT_LAYER;
		...
		protected void updateUndoAction(CustomEmbeddedEditorActions actions) {
		...
		protected OperationHistoryListener installUndoRedoSupport(SourceViewer viewer, IDocument document,
				final CustomEmbeddedEditorActions actions) {
		...
		protected CustomEmbeddedEditorActions initializeActions(final SourceViewer viewer) {
